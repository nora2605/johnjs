class John {
    ...
    *tokens() {
    ...
    stringify(options = format_default) {
        let result = '';
        this.tokenizer.next();
        let inside = ['tlo'];
        let last = () => inside[inside.length - 1];
        let newliners = () => {
            let newliners = [];
            if (options.array_delimeter.includes('\n')) newliners.push('[');
            if (options.tuple_delimeter.includes('\n')) newliners.push('(');
            if (options.object_delimeter.includes('\n')) newliners.push('{');
            if (options.set_delimeter.includes('\n')) newliners.push('{[');
            if (options.dict_delimeter.includes('\n')) newliners.push('{{');
            if (options.node_attribute_delimeter.includes('\n')) newliners.push('|');
            return newliners;
        };
        let end_strippers = () => {
            let end_strippers = [];
            if (!options.array_delimeter_at_end) end_strippers.push(']');
            if (!options.tuple_delimeter_at_end) end_strippers.push(')');
            if (!options.object_delimeter_at_end) end_strippers.push('}');
            if (!options.set_delimeter_at_end) end_strippers.push(']}');
            if (!options.dict_delimeter_at_end) end_strippers.push('}}');
            if (!options.node_attribute_delimeter_at_end) end_strippers.push('|');
            return end_strippers;
        };
        let delimeter_length = (brace) => {
            switch (brace) {
                case '[':
                    return options.array_delimeter.length;
                case '(':
                    return options.tuple_delimeter.length;
                case '{':
                    return options.object_delimeter.length;
                case '{[':
                    return options.set_delimeter.length;
                case '{{':
                    return options.dict_delimeter.length;
                case '|':
                    return options.node_attribute_delimeter.length;
                default:
                    return 0;
            }
        }
        ...
        while (this.cur_token) {
            let token = this.cur_token;
            let current_indent_width = options.indent_size * (inside.filter(x => newliners().includes(x)).length);
            result += current_indent_width > 0 && is_key ? ' '.repeat(current_indent_width) : '';
            result += token;
            if (token === '{' || token === '[' || token === '(' || token === '|') {
                is_key = true;
                is_attribute = false;
                this.tokenizer.next();
                if (token === '{' && this.cur_token === '[') {
                    result += this.cur_token;
                    this.tokenizer.next();
                    inside.push('{[');
                }
                else if (token === '{' && this.cur_token === '{') {
                    result += this.cur_token;
                    this.tokenizer.next();
                    inside.push('{{');
                }
                ...
                if (newliners().includes(token)) {
                    result += '\n';
                }
            } else if (token === '}' || token === ']' || token === ')') {
                let nl = (newliners().includes(last()) ? '\n' : '');
                if (end_strippers().includes(token))
                    result = result.slice(0, -delimeter_length(inside.pop())-1-current_indent_width) + nl + token;
                else inside.pop();
                is_key = false;
                this.tokenizer.next();
                if (token === '}' && this.cur_token === '}' && last() === '{{') {
                    result += this.cur_token;
                    this.tokenizer.next();
                } else if (token === ']' && this.cur_token === '}' && last() === '{[') {
                    result += this.cur_token;
                    this.tokenizer.next();
                }
            } else {
                switch (last()) {
                    ...
                    case '{':
                        if (is_key) {
                            result += options.object_key_delimeter;
                        } else {
                            result += options.object_delimeter;
                        }
                        is_key = !is_key;
                        break;
                    case '[':
                        result += options.array_delimeter;
                        break;
                    case '(':
                        result += options.tuple_delimeter;
                        break;
                    case '|':
                        if (is_attribute) {
                            result += options.node_attribute_delimeter;
                        } else {
                            result += options.node_attribute_key_delimeter;
                        }
                        is_attribute = !is_attribute;
                        break;
                    case '{{':
                        if (is_key) {
                            result += options.dict_key_delimeter;
                        } else {
                            result += options.dict_delimeter;
                        }
                        is_key = !is_key;
                        break;
                    case '{[':
                        result += options.set_delimeter;
                        break;
                    default:
                        console.log('what.');
                        break;
        ...
        return options.top_level_object ? result : '{\n' + result.split('\n').map(x => ' '.repeat(indent_size).join('\n') + x) + '\n}';
    }
}
...
function stringify(input) {
    return _stringify(input, true);
}

function _stringify(input, toplevel = false) {
    ...
    if (Array.isArray(input)) {
        ...
        for (let i = 0; i < input.length; i++) {
            johntext += _stringify(input[i]);
            if (i < input.length - 1) johntext += ' ';
        }
        johntext += ']';
        return johntext;
    }
    if (input instanceof Map) {
        ...
        for (let [key, value] of input) {
            johntext += _stringify(key) + ' ' + _stringify(value) + ' ';
        }
        johntext = johntext.slice(0, -1);
        johntext += '}}';
        return johntext;
    }
    ...
    if (typeof input === 'object') {
        let johntext = '';
        if (!toplevel) johntext += '{';
        for (let key of Object.keys(input)) {
            let value = _stringify(input[key]);
            if (!value) continue;
            johntext += key + ' ' + value + ' ';
        }
        johntext = johntext.slice(0, -1);
...
function format(input, options) {
    return new John(input).stringify(options);
}

const format_default = {
    indent_size: 4,
    array_delimeter: ', ',
    array_delimeter_at_end: false,
    tuple_delimeter: '; ',
    tuple_delimeter_at_end: false,
    object_key_delimeter: ': ',
    object_delimeter: ',\n',
    object_delimeter_at_end: false,
    set_delimeter: ', ',
    set_delimeter_at_end: false,
    dict_key_delimeter: ': ',
    dict_delimeter: ',\n',
    dict_delimeter_at_end: false,
    node_attribute_delimeter: ' ',
    node_attribute_key_delimeter: '=',
    node_attribute_delimeter_at_end: false,
...
module.exports = { parse, stringify, format, format_default, format_minify };