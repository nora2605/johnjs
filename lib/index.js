class John {
    ...
    *tokens() {
    ...
    stringify(options = format_default) {
        ...
        let newliners = () => {
            let newliners = [];
            if (options.array_delimeter === '\n') newliners.push('[');
            if (options.tuple_delimeter === '\n') newliners.push('(');
            if (options.object_delimeter === '\n') newliners.push('{');
            if (options.set_delimeter === '\n') newliners.push('{[');
            if (options.dict_delimeter === '\n') newliners.push('{{');
            if (options.node_attribute_delimeter === '\n') newliners.push('|');
            return newliners;
        };
        let end_strippers = () => {
            let end_strippers = [];
        ...
        while (this.cur_token) {
            let token = this.cur_token;
            let current_indent_width = options.indent_size * (inside.filter(x => newliners().includes(x)).length - 1);
            result += current_indent_width > 0 ? ' '.repeat(current_indent_width) : '';
            result += token;
            if (token === '{' || token === '[' || token === '(' || token === '|') {
                ...
                if (end_strippers().includes(token))
                    result = result.slice(0, -delimeter_length(inside.pop())-1) + token;
                else inside.pop();
                is_key = false;
                this.tokenizer.next();
