...
function parse_tokens(tokens) {
  ...
  if (tokens[0][1] !== token_types.identifier) {
    throw new JOHNError(
      `Parser Error: Line ${
        tokens[0][2]
      }: Expected an identifier, instead found ${tokens[0][1]}`,
    );
  }
  ...
  while (i < tokens.length) {
    if (!tokens[i]) {
      throw new JOHNError(
        `Parser error: Line ${tokens[i][2]}: Expected object content`,
      );
    }
    ...
    switch (tokens[i][1]) {
      case token_types.identifier:
        throw new JOHNError(
          `Parser error: Line ${tokens[i][2]}: Invalid object content: "${
            tokens[i][0]
          }"`,
        );
      ...
      case token_types.right_square_brace:
        ...
        throw new JOHNError(
          `Parser error: Line ${tokens[i][2]}: Unopened "${
            tokens[i][0]
          }" found`,
        );
      case token_types.link:
        if (!identifier) {
          throw new JOHNError(
            `Parser error: Line ${tokens[i][2]}: Unexpected link symbol`,
          );
        }
        i++;
        let llist = [];
        while (
          tokens[i] && tokens[i][1] !== token_types.identifier &&
          tokens[i][1] !== token_types.link
        ) {
          switch (tokens[i][1]) {
      ...
      case token_types.left_curly_brace:
        ...
        john_object[identifier] = parse_tokens(
          tokens.slice(i + 1, closing_index),
        );
      ...
      case token_types.left_square_brace:
        ...
        closing_index = find_matching_bracket(tokens, i);
        // Parse the array
        john_object[identifier] = parse_array(
          tokens.slice(i + 1, closing_index),
        );
        // Next index is index after the closing curly brace
        i = closing_index + 1;
...
function find_matching_bracket(tokens, index) {
  ...
  let rightb = leftb === token_types.left_curly_brace
  ...
  throw new JOHNError(
    `Parser error: Line ${
      tokens[index][2]
    }: No matching bracket found for token ${index}`,
  );
}
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (tokens[i][1] === token_types.identifier) {
      throw new JOHNError(
        `Parser error: Line ${
          tokens[i][2]
        }: Identifiers can't be inside an array`,
      );
    }
    switch (tokens[i][1]) {
      case token_types.right_curly_brace:
      ...
      case token_types.link:
        throw new JOHNError(
          `Parser error: Line ${tokens[i][2]}: Unexpected link symbol in array`,
        );
      case token_types.object:
...
function parse_object(token) {
  ...
  // any number type
  let dmatches =
    (/^-?(?:(?:\d+)|(?:\d*\.\d+))([ui](?:8|16|32|64)|f(?:32|64))?$/).exec(
      token,
    );
  if (dmatches) {
  ...
  if ((/^v(?:\d+\.){2,3}\d+$/).test(token)) {
    ...
    return {
      ...
      build: version_elements.length > 3
        ? Number.parseInt(version_elements[2])
        : undefined, // if index out of range just remains undefined
    };
  }
  // index
  if ((/^([\*\^])\d+/).test(token)) {
  ...
  if ((/^\d+(?:\.\d+)?\.\.\d+(?:\.\d+)?(?:\.\.\d+(?:\.\d+)?)?$/).test(token)) {
    ...
    let range = [];
    for (
      let i = range_elements[0];
      i < range_elements[1];
      i += range_elements[2] ?? 1
    ) {
      range.push(i);
    }
    return range;
...
function tokenize(johntext) {
  ...
  for (let i = 0; i < input.length; i++) {
    switch (input.charAt(i)) {
        ...
        if (!str) {
          if (chr) {
            ...
            if (currentToken.length > 3) {
              throw new JOHNError(
                `Tokenizer error: Line ${line}: Char cannot contain more than 1 character. Consider using a string literal instead.`,
              );
            }
            tokens.push([currentToken, line]);
            currentToken = "";
      ...
      case "\n":
        if (str || chr) {
          throw new JOHNError(
            `Tokenizer error: Line ${line}: Literal may not contain a newline. To include a newline use \\n instead.`,
          );
        }
        if (currentToken) {
          tokens.push([currentToken, line]);
  ...
  if (str || chr) {
    const unmatchedQuoteLine = 1 + input.split("")
      ...
      .length;
    throw new JOHNError(
      `Tokenizer Error: Unmatched quote at line ${unmatchedQuoteLine}`,
    );
  }
...
exports.minify = (johntext) => {
  return tokenize(johntext).map((t) => t[0]).join(" ").replace(
    /\s*[\[\{\(\]\}\)]\s*/g,
    (s) => s.replace(/\s+/g, ""),
  );
};
